...
test('Light combo damage lands and a 3-hit chain increments combo missions',()=>{const e=engine();for(let i=0;i<3;i++){e.e.x=e.p.x+90;e.input('light');tick(e,.47);}assert.ok(e.e.hp<e.e.maxHp);assert.ok(e.comboCount>=1);assert.ok(e.p.energy>=30);});
